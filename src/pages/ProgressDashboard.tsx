    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
          ...
          AlgoForge Visualizations Progress
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          >
            <div className="text-center">
              <p className="text-2xl font-bold text-green-400 mb-1">{completionCount}</p>
              <p className="text-sm text-slate-400">Problems Completed</p>
              <p className="text-xs text-slate-500 mt-1">
                {Math.round((completionCount / problems.length) * 100)}% of all problems
              </p>
            </div>
          </motion.div>
          <motion.div
          ...
          >
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-400 mb-1">
                {problems.filter(p => p.hasVisualization && completions.includes(p.id)).length}
              </p>
              <p className="text-sm text-slate-400">Visualized & Completed</p>
              <p className="text-xs text-slate-500 mt-1">
                {Math.round((problems.filter(p => p.hasVisualization && completions.includes(p.id)).length / problems.filter(p => p.hasVisualization).length) * 100)}% of visualized problems
              </p>
            </div>
          </motion.div>
          <motion.div
          ...
          >
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-400 mb-1">{categories.length}</p>
              <p className="text-sm text-slate-400">Categories</p>
              <p className="text-xs text-slate-500 mt-1">
                {categories.filter(cat => getCompletionPercentage(cat) > 0).length} with progress
              </p>
            </div>
          </motion.div>
          <motion.div
          ...
          >
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-400 mb-1">
                {Math.round(categories.reduce((sum, cat) => sum + getCompletionPercentage(cat), 0) / categories.length)}%
              </p>
        ...
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {categories.map((category, index) => {
              const percentage = getCompletionPercentage(category);
              const categoryProblems = problems.filter(p => p.category === category);
              const completed = categoryProblems.filter(p => completions.includes(p.id));
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-slate-400">
                        {completed.length}/{categoryProblems.length}
                      </span>
                      <span
                        className={`text-xs font-bold px-2 py-1 rounded ${
                          percentage >= 80
