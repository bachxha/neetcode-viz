import { useState, useEffect, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Controls } from '../components/Controls';

interface Interval {
  start: number;
  end: number;
  id: number;
  color: string;
}

interface Step {
  type: 'initial' | 'sort' | 'compare' | 'merge' | 'add' | 'done';
  intervals: Interval[];
  result: Interval[];
  currentIndex: number;
  compareIndex: number | null;
  description: string;
  highlightResult: number | null;
}

const COLORS = [
  '#3b82f6', '#22c55e', '#eab308', '#ef4444', '#a855f7',
  '#ec4899', '#14b8a6', '#f97316', '#6366f1', '#84cc16',
];

function parseIntervals(input: string): Interval[] {
  try {
    const parsed = JSON.parse(input);
    return parsed.map((arr: number[], i: number) => ({
      start: arr[0],
      end: arr[1],
      id: i,
      color: COLORS[i % COLORS.length],
    }));
  } catch {
    return [];
  }
}

function generateSteps(intervals: Interval[]): Step[] {
  if (intervals.length === 0) return [];
  
  const steps: Step[] = [];
  
  // Initial state
  steps.push({
    type: 'initial',
    intervals: [...intervals],
    result: [],
    currentIndex: -1,
    compareIndex: null,
    description: 'Initial intervals (unsorted)',
    highlightResult: null,
  });
  
  // Sort by start time
  const sorted = [...intervals].sort((a, b) => a.start - b.start);
  steps.push({
    type: 'sort',
    intervals: sorted,
    result: [],
    currentIndex: -1,
    compareIndex: null,
    description: 'Sort intervals by start time',
    highlightResult: null,
  });
  
  // Process each interval
  const result: Interval[] = [{ ...sorted[0] }];
  steps.push({
    type: 'add',
    intervals: sorted,
    result: [...result],
    currentIndex: 0,
    compareIndex: null,
    description: `Add first interval [${sorted[0].start}, ${sorted[0].end}] to result`,
    highlightResult: 0,
  });
  
  for (let i = 1; i < sorted.length; i++) {
    const current = sorted[i];
    const lastResult = result[result.length - 1];
    
    steps.push({
      type: 'compare',
      intervals: sorted,
      result: [...result],
      currentIndex: i,
      compareIndex: result.length - 1,
      description: `Compare [${current.start}, ${current.end}] with last result [${lastResult.start}, ${lastResult.end}]`,
      highlightResult: result.length - 1,
    });
    
    if (current.start <= lastResult.end) {
      // Merge
      lastResult.end = Math.max(lastResult.end, current.end);
      lastResult.color = current.color; // Take the color of merged interval
      steps.push({
        type: 'merge',
        intervals: sorted,
        result: result.map(r => ({ ...r })),
        currentIndex: i,
        compareIndex: result.length - 1,
        description: `Overlap! Merge into [${lastResult.start}, ${lastResult.end}]`,
        highlightResult: result.length - 1,
      });
    } else {
      // Add new
      result.push({ ...current });
      steps.push({
        type: 'add',
        intervals: sorted,
        result: result.map(r => ({ ...r })),
        currentIndex: i,
        compareIndex: null,
        description: `No overlap. Add [${current.start}, ${current.end}] to result`,
        highlightResult: result.length - 1,
      });
    }
  }
  
  steps.push({
    type: 'done',
    intervals: sorted,
    result: result.map(r => ({ ...r })),
    currentIndex: -1,
    compareIndex: null,
    description: `Done! ${result.length} merged interval${result.length !== 1 ? 's' : ''}`,
    highlightResult: null,
  });
  
  return steps;
}

export function MergeIntervalsVisualizer() {
  const [inputValue, setInputValue] = useState('[[1,3],[2,6],[8,10],[15,18]]');
  const [intervals, setIntervals] = useState<Interval[]>([]);
  const [steps, setSteps] = useState<Step[]>([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1);
  
  useEffect(() => {
    const parsed = parseIntervals(inputValue);
    if (parsed.length > 0) {
      setIntervals(parsed);
      const newSteps = generateSteps(parsed);
      setSteps(newSteps);
      setCurrentStep(0);
      setIsPlaying(false);
    }
  }, []);
  
  const handleInputChange = useCallback(() => {
    const parsed = parseIntervals(inputValue);
    if (parsed.length > 0 && parsed.length <= 10) {
      setIntervals(parsed);
      const newSteps = generateSteps(parsed);
      setSteps(newSteps);
      setCurrentStep(0);
      setIsPlaying(false);
    }
  }, [inputValue]);
  
  useEffect(() => {
    if (!isPlaying || currentStep >= steps.length - 1) {
      if (currentStep >= steps.length - 1) setIsPlaying(false);
      return;
    }
    
    const timer = setTimeout(() => {
      setCurrentStep(s => s + 1);
    }, 1000 / speed);
    
    return () => clearTimeout(timer);
  }, [isPlaying, currentStep, steps.length, speed]);
  
  const currentStepData = steps[currentStep];
  const maxEnd = Math.max(...intervals.map(i => i.end), 20);
  const scale = 700 / maxEnd;
  
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Merge Intervals</h1>
        <p className="text-slate-400">
          Merge overlapping intervals. Watch the sort and sweep process.
        </p>
      </div>
      
      <div className="mb-6 flex gap-4 items-center">
        <label className="text-sm text-slate-400">Input intervals:</label>
        <input
          type="text"
          value={inputValue}
          onChange={(e) => setInputValue(e.target.value)}
          onBlur={handleInputChange}
          onKeyDown={(e) => e.key === 'Enter' && handleInputChange()}
          className="px-3 py-2 bg-slate-800 rounded-lg border border-slate-600 focus:border-blue-500 outline-none flex-1 font-mono text-sm"
          placeholder="[[1,3],[2,6],[8,10],[15,18]]"
        />
      </div>
      
      {/* Timeline Visualization */}
      <div className="bg-slate-800 rounded-lg p-6 mb-6">
        <h3 className="text-sm font-semibold text-slate-400 mb-4">
          {currentStepData?.type === 'initial' ? 'Original Intervals' : 'Sorted Intervals'}
        </h3>
        
        <svg viewBox="0 0 800 200" className="w-full h-48">
          {/* Timeline axis */}
          <line x1="40" y1="150" x2="760" y2="150" className="stroke-slate-600" />
          {Array.from({ length: maxEnd + 1 }, (_, i) => (
            <g key={i}>
              <line x1={40 + i * scale} y1="145" x2={40 + i * scale} y2="155" className="stroke-slate-600" />
              <text x={40 + i * scale} y="170" textAnchor="middle" className="fill-slate-500 text-xs">
                {i}
              </text>
            </g>
          ))}
          
          {/* Intervals */}
          <AnimatePresence>
            {currentStepData?.intervals.map((interval, i) => {
              const isCurrentIndex = currentStepData.currentIndex === i;
              const y = 30 + (i % 4) * 28;
              
              return (
                <motion.g key={interval.id}>
                  <motion.rect
                    x={40 + interval.start * scale}
                    y={y}
                    width={(interval.end - interval.start) * scale}
                    height={20}
                    rx={4}
                    fill={interval.color}
                    initial={{ opacity: 0, scaleX: 0 }}
                    animate={{ 
                      opacity: isCurrentIndex ? 1 : 0.7,
                      scaleX: 1,
                      strokeWidth: isCurrentIndex ? 3 : 0,
                    }}
                    style={{ 
                      originX: 0,
                      stroke: isCurrentIndex ? '#fff' : 'none',
                    }}
                    transition={{ duration: 0.3 }}
                  />
                  <motion.text
                    x={40 + interval.start * scale + (interval.end - interval.start) * scale / 2}
                    y={y + 14}
                    textAnchor="middle"
                    className="fill-white text-xs font-mono font-bold"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  >
                    [{interval.start},{interval.end}]
                  </motion.text>
                </motion.g>
              );
            })}
          </AnimatePresence>
        </svg>
      </div>
      
      {/* Result */}
      <div className="bg-slate-800 rounded-lg p-6 mb-6">
        <h3 className="text-sm font-semibold text-slate-400 mb-4">Merged Result</h3>
        
        <svg viewBox="0 0 800 80" className="w-full h-20">
          {/* Timeline axis */}
          <line x1="40" y1="60" x2="760" y2="60" className="stroke-slate-600" />
          
          {/* Result intervals */}
          <AnimatePresence>
            {currentStepData?.result.map((interval, i) => {
              const isHighlighted = currentStepData.highlightResult === i;
              
              return (
                <motion.g key={`result-${i}`}>
                  <motion.rect
                    x={40 + interval.start * scale}
                    y={15}
                    width={(interval.end - interval.start) * scale}
                    height={28}
                    rx={4}
                    fill={isHighlighted ? '#22c55e' : '#3b82f6'}
                    initial={{ opacity: 0, scaleX: 0 }}
                    animate={{ 
                      opacity: 1, 
                      scaleX: 1,
                      fill: isHighlighted ? '#22c55e' : '#3b82f6',
                    }}
                    style={{ originX: 0 }}
                    transition={{ duration: 0.3 }}
                  />
                  <motion.text
                    x={40 + interval.start * scale + (interval.end - interval.start) * scale / 2}
                    y={34}
                    textAnchor="middle"
                    className="fill-white text-sm font-mono font-bold"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  >
                    [{interval.start},{interval.end}]
                  </motion.text>
                </motion.g>
              );
            })}
          </AnimatePresence>
          
          {currentStepData?.result.length === 0 && (
            <text x="400" y="35" textAnchor="middle" className="fill-slate-500">
              No merged intervals yet
            </text>
          )}
        </svg>
      </div>
      
      {/* Status */}
      <div className="bg-slate-800 rounded-lg p-4 mb-6">
        <div className="flex items-center gap-2">
          <span className={`w-3 h-3 rounded-full ${
            currentStepData?.type === 'merge' ? 'bg-green-500' :
            currentStepData?.type === 'compare' ? 'bg-yellow-500' :
            currentStepData?.type === 'add' ? 'bg-blue-500' :
            currentStepData?.type === 'done' ? 'bg-purple-500' :
            'bg-slate-500'
          }`} />
          <span className="text-lg">{currentStepData?.description || 'Ready'}</span>
        </div>
      </div>
      
      <Controls
        isPlaying={isPlaying}
        onPlayPause={() => setIsPlaying(!isPlaying)}
        onStepBack={() => setCurrentStep(s => Math.max(0, s - 1))}
        onStepForward={() => setCurrentStep(s => Math.min(steps.length - 1, s + 1))}
        onReset={() => { setCurrentStep(0); setIsPlaying(false); }}
        currentStep={currentStep + 1}
        totalSteps={steps.length}
        speed={speed}
        onSpeedChange={setSpeed}
        canStepBack={currentStep > 0}
        canStepForward={currentStep < steps.length - 1}
      />
      
      {/* Code Reference */}
      <div className="mt-6 bg-slate-800 rounded-lg p-4">
        <h3 className="text-sm font-semibold text-slate-400 mb-2">Java Code</h3>
        <pre className="text-sm font-mono text-slate-300 overflow-x-auto">
{`public int[][] merge(int[][] intervals) {
    if (intervals.length == 0) return new int[0][];
    
    // Sort by start time
    Arrays.sort(intervals, (a, b) -> a[0] - b[0]);
    
    List<int[]> result = new ArrayList<>();
    result.add(intervals[0]);
    
    for (int i = 1; i < intervals.length; i++) {
        int[] current = intervals[i];
        int[] last = result.get(result.size() - 1);
        
        if (current[0] <= last[1]) {  // Overlap
            last[1] = Math.max(last[1], current[1]);  // Merge
        } else {
            result.add(current);  // No overlap, add new
        }
    }
    
    return result.toArray(new int[result.size()][]);
}`}
        </pre>
      </div>
    </div>
  );
}
