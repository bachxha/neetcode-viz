...
function generateSteps(heights: number[]): Step[] {
  ...
  steps.push({
    ...
    maxArea: 0,
    steps: 0,
    description: `Two pointers: Start at both ends`,
  });
  ...
  while (left < right) {
    ...
    steps.push({
      ...
      maxArea,
      steps: stepCount,
      description: `Area = ${width} × ${height} = ${area}`,
    });
    ...
    if (heights[left] < heights[right]) {
      ...
      steps.push({
        ...
        maxArea,
        steps: stepCount,
        description: `Move left → ${left} (was shorter)`,
      });
    } else {
      right--;
      steps.push({
        ...
        maxArea,
        steps: stepCount,
        description: `Move right → ${right} (was shorter)`,
      });
    }
  }
  ...
  steps.push({
    ...
    maxArea,
    steps: stepCount,
    description: `Done! Max area = ${maxArea}. Only ${stepCount} iterations!`,
  });
...
export function ContainerTwoPointersVisualizer() {
      ...
      <div className="bg-slate-900 rounded-lg p-4 mb-4 flex-1">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 text-center">
          ...
          <div className="bg-blue-500/10 border border-blue-500/30 rounded p-2">
            <div className="text-blue-400 font-bold text-lg">{step.maxArea}</div>
        ...
        <div className="flex items-end justify-center gap-1 mb-4" style={{ height: barHeight + 30 }}>
          {step.heights.map((h, idx) => {
            ...
            const inRange = step.left >= 0 && step.right >= 0 && idx >= step.left && idx <= step.right;
            const isOutside = step.left >= 0 && (idx < step.left || idx > step.right);
            ...
            const waterHeight = showWater && inRange 
              ? (Math.min(step.heights[step.left], step.heights[step.right]) / maxHeight) * barHeight 
              : 0;
              ...
              <div key={idx} className="flex flex-col items-center relative">
                ...
                {isLeft && (
                  <motion.div 
                    ...
                    className="absolute -top-5 text-blue-400 font-bold text-xs"
                  >
                    L
                  </motion.div>
                )}
                {isRight && (
                  <motion.div 
                    initial={{ y: -5, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    className="absolute -top-5 text-cyan-400 font-bold text-xs"
                  >
                    R
                  </motion.div>
                )}
        ...
        {/* Current calculation */}
        {(step.type === 'calculate' || step.type === 'update-max') && step.left >= 0 && step.right >= 0 && (
          <motion.div 
          ...
          >
            <span className={`px-3 py-1.5 rounded-lg font-mono text-sm ${
              ...
              Area = {step.right - step.left} × {Math.min(step.heights[step.left], step.heights[step.right])} = {step.currentArea}
            </span>
          </motion.div>
        )}
      </div>
      ...
      <div className="bg-slate-800 rounded-lg p-3 mb-4">
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${
            ...
            step.type === 'move-left' ? 'bg-blue-500' :
            step.type === 'move-right' ? 'bg-cyan-500' :
            'bg-yellow-500'
          }`} />
